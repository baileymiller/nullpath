...
assemble option:
    ...
    --averageduplicate Average duplicate pixels instead of discarding.
...
int assemble(int argc, char *argv[]) {
    ...
    for (int i = 0; i < argc; ++i) {
        if (!strcmp(argv[i], "--outfile") || !strcmp(argv[i], "-outfile")) {
            ...
            outfile = &argv[i][10];
        } else if (!strcmp(argv[i], "--averageduplicate") || !strcmp(argv[i], "-averageduplicate")) {
            averageDuplicate = true;
        } else
    ...
    for (const char *file : infiles) {
        ...
        if (!fullImg) {
            ...
            numPixels.resize(fullRes.x * fullRes.y, 0);
            displayWindow = dspw;
        } else {
        ...
        for (int y = 0; y < res.y; ++y)
            for (int x = 0; x < res.x; ++x) {
                ...
                if (numPixels[fullOffset] > 0) ++seenMultiple;
                ++numPixels[fullOffset];
            }
    }
    ...
    for (int y = 0; y < fullRes.y; ++y)
        for (int x = 0; x < fullRes.x; ++x) {
            int idx = y * fullRes.x + x;
            int n = numPixels[idx];
            if (n == 0) ++unseenPixels;
            else if (averageDuplicate) {
                fullImg[idx] /= n;
            }
        }
